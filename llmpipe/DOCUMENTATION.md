...
an ordinary gk proof and goes through the same `process_proof`; only the rendering names
...
│   ├── litbridge_atoms.py      Literal-bridge: candidate atoms, display, the raw Stage-2 reading (ch. 13)
│   ├── litbridge_rules.py      Literal-bridge: rule grammar + the distinctness and negative-relation channels
│   ├── litbridge_compile.py    Literal-bridge: rule → clauses (converter route, exact templates, redundancy check)
│   ├── litbridge_chain.py      Literal-bridge: reading a compiled bridge back (suppliers, chain status, what a proof cites)
...
folded into *Knowledge used* (§13.6).
...
described together in §13 rather than one subsection each, because they are only ever
used as one stack:
...
The stack is acyclic; `litbridge_atoms` imports only `litbridge_converter`.  Nothing in
...
**Mode option keys** (defaults as noted; see §11, §12 and §13 for what they do):
...
`existfold_flag`, `noproptypes_flag`, `propclass_flag`, `numtype_flag`,
`compasym_flag`), `prenorm_flag`, `nominalretry_flag`, `negretry_flag`,
`crossstage_retry_flag`
...
## 13. Literal-bridge abstraction (`-litbridge`)
...
### 13.3 Compiling a rule to clauses
...
whichever route wins.  A **channel rule** (§13.4) is not `governs()`: it takes
...
(`memos/MEMO_2026_08_10_bridge_world.md` §5).  A rule whose compiled clause the theory
...
### 13.5 The encoding the bridge is converted under
...
behind the design, are in `memos/MEMO_2026_08_16_litbridge_encoding.md`.

### 13.6 What the output shows
...
`litbridge_*` files.  `memos/MEMO_2026_08_15_litbridge_merge.md` records that merge, the
seven defect classes a version-chain collapse hides, and the oracles that check it:
`tools/litbridge_replay_oracle.py` recompiles every archived rule of the closed arms and

...
For the underlying data representations see [encodings](../encodings/README.md).
For the modules that produce this output see
[proof processing](../code/proof-processing.md).
...
1. [Overview](#overview)
2. [Entity Naming](#entity-naming)
3. [Atom-to-English Translation](#atom-to-english-translation)
4. [Clause Rendering](#clause-rendering)
5. [Proof Explanation Structure](#proof-explanation-structure)
6. [Logic Display Modes](#logic-display-modes)
7. [Examples](#examples)
...
re-exports their public API; see [the source map](../code/source-map.md) for the full
...
vocabulary the heuristics of [the source map](../code/source-map.md) were written for.  `looks_like_verb` reads
...
| `["$block",...]` | See [the encoding reference](../encodings/README.md) |
| `false` | `Contradiction` |
...
with `-json` (see [the prompt map](../code/prompt-map.md)).

---

## Logic Display Modes

### Traditional Syntax (default)
...
### Defeasible Proof with Exception Check
...
### Wh-Question with Background Knowledge
...
### Event Proof with Relative Clause

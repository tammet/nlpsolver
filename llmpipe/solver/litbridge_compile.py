...
def match_template(atom, group, index, rule_vars, bindings, generalised,
  ...
  label_slot = atoms.LABEL_SLOT.get(predicate)
  subst = {}
...
def build_world(world_id, rules, view, configuration, groups=(), weight=1.0,
                check_redundancy=False, hypothesis_case_id=None):
  """v6's world, compiled through the exact-template route.

  `check_redundancy` refuses a rule whose compiled clause the theory already
  contains.  It is on for a submission to gk and off for the chain reading,
  which compiles only to look at body and head literals.

  `hypothesis_case_id` replaces the view's own case id in every clause name,
  so a caller that compiles more than one round can tell the rounds apart in
  the proof gk returns.
  """
  got = _build_world_base(world_id, rules, view, configuration,
                          groups=groups, weight=weight,
                          check_redundancy=check_redundancy,
                          hypothesis_case_id=hypothesis_case_id)
...
def _match_display(atom, group, index, rule_vars, bindings, generalised):
  ...
  names = {}
  args = [_standardise(a, index, names) for a in template[1:1 + n]]
  label_slot = atoms.LABEL_SLOT.get(str(display[0]))
  predicate = str(display[0])
  subst = {}
  for i, (written, shown) in enumerate(zip(atom[1:], display[1:])):
    slot = args[i]
...
def _looks_like_a_world(term):
  return isinstance(term, str) and bool(atoms.is_stage2_variable(term)) \
...
def _build_world_base(world_id, rules, view, configuration, groups=(), weight=1.0,
  ...
  owner = hypothesis_case_id or view["case_id"]
  for r in rules:
    hid = "%s::%s" % (owner, r["rule_id"])
    try:
      ...
      continue
    if check_redundancy:
      already = already_present(cl, view.get("final_clauses") or [])
      if already:
...
# `$block` guard.  Broad semantic subsumption is a different question and is
# not attempted here — a rule that merely follows from the theory is still a
# rule the theory does not contain, and letting it through costs one clause.

BLOCK = "$block"

MAX_CLAUSE_LITERALS_COMPARED = 6    # 6! orderings; a longer clause is not matched
...
def alpha_equal_clause(a, b):
  ...
  if len(a) != len(b) or not a:
    return False
  if len(a) > MAX_CLAUSE_LITERALS_COMPARED:
    return False
